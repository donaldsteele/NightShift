...
{
    readonly string _root = Path.Combine(Path.GetTempPath(), "nightshift-tests", Guid.NewGuid().ToString("n"));
    ...
    public void Layout_matches_the_documented_shape()
    {
        ...
        Assert.Equal(Path.Combine(_root, "logs", "nightshift-.log"), paths.LogFileTemplate);
